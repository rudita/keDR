...
abstract class BaseCacheDecorator implements BaseRepository
{
    ...
    {
        $filtered = json_encode($filter);
        $sorted = json_encode($sort);
        return $this->cache
            ->tags($this->entityName, 'global')
            ->remember("{$this->locale}.{$this->entityName}.fetch.{$filtered}.{$sorted}.{$perPage}.{$page}", $this->cacheTime,
                function () use ($filter, $sort, $perPage, $page) {
                    return $this->repository->fetch($filter, $sort, $perPage, $page);
                }
            );
    }
}